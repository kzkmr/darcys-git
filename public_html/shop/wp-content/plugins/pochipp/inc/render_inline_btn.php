 ...
 * インラインボタン出力用
 */
function get_inline_btn( $pid, $shop, $content, $pdata = [] ) {
...
function render_pochipp_btn( $btn_data = [] ) {
	...
	if ( 'yahoo' === $shop ) {
		...
		// 追加クラス
		$add_class = $is_paypay ? 'pochipp-inline__btnwrap -paypay' : 'pochipp-inline__btnwrap -yahoo';
	}
	if ( 'custom1' === $shop ) {
		$url = $custom_btn_url;
		// ボタンテキスト
